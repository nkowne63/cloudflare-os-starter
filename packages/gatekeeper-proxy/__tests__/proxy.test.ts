...
const serviceJson = JSON.stringify({
  ...
  api: { upstream: "https://api.example.com", via: "public", writeMethods: "allow" },
});
...
describe("proxy service configuration and resources", () => {
  it("exposes only ESM in the built-in default configuration", () => {
    expect([...DEFAULT_PROXY_SERVICES.keys()]).toEqual(["esm"]);
    expect(DEFAULT_PROXY_SERVICES.get("esm")).toMatchObject({
      upstream: "https://esm.sh",
      via: "public",
      writeMethods: "deny",
    });
    expect(DEFAULT_PROXY_SERVICES.has("grafana")).toBe(false);
  });

  it("resolves any explicitly configured API service without wildcard routing", () => {
    const services = parseProxyServices(JSON.stringify({
      payments: { upstream: "https://payments.example.test/v1", via: "public", writeMethods: "deny" },
    }));
    expect(resolveProxyResource("proxy://payments", services).config.upstream)
      .toBe("https://payments.example.test/v1");
    expect(() => resolveProxyResource("proxy://payments.example.test", services)).toThrow();
  });
...
describe("proxy request policy and transport", () => {
  ...
  it("keeps public writes deny-by-default and never prompts for allow", async () => {
    ...
    const allowed = new ProxySessionImpl(allowedQueue, { service: "api", config: parseProxyServices(serviceJson).get("api")! }, { fetch });
  ...
  it("enforces the 8 MiB body and response boundary and preserves base64 bodies", async () => {
    ...
    const session = new ProxySessionImpl(q, { service: "api", config: parseProxyServices(serviceJson).get("api")! }, { fetch });
    ...
    const oversizedResponse = new ProxySessionImpl(q, { service: "api", config: parseProxyServices(serviceJson).get("api")! }, {
